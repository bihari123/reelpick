...
pub fn main() !void {
    ...
    const queue = try RedisJobQueue.init(allocator, "localhost", 6379, 3);
    defer queue.deinit();

    // Example job with proper memory management
    const example_job = try Job.init(allocator, "test_job_001", "example data payload");
